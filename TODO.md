...
## 🔒 현재 편집 중
- 작업 중: slides-grab Codex 설치 가이드 적용 (npm/Playwright/공유 스킬 설치)
- 작업 중(Claude Code, 2026-08-08): `src/drive_pkg/` + `src/slam_pkg/launch/` + `scripts/` + `docker/` — Jetson 실측 준비 구현 (계획: `docs/superpowers/plans/2026-08-08-jetson-field-mapping.md`) — Codex는 이 경로 동시 편집 금지
...
## 메모
- 2026-08-09: **Jetson 실측 준비 구현 세션 (스펙+계획+Task 1~10 완료).** 스펙 `docs/superpowers/specs/2026-08-08-jetson-field-mapping-design.md`(v2, Codex 리뷰 반영) → 계획 `docs/superpowers/plans/2026-08-08-jetson-field-mapping.md` → Inline 실행. 산출물: OpenCR 프로토콜 문서/모듈, 오도메트리, 브리지 노드(+0.5s watchdog), teleop dead-man, 실기 매핑 launch(RSP/RViz 조건부, dry-run 실기동 검증), `save_kku_map --real`+posegraph, `run_field_mapping.sh`(에러경로 실검증), `scripts/udev/`, jetson compose 최소권한화(devices+SYS_NICE+logs 마운트), publish-ghcr.yml에 jetson 잡 추가. 게이트: 오프라인 25 PASS/0 FAIL, 이식성 96파일 PASS, 컨테이너 colcon+dry-run PASS. **블로커(사용자 액션 대기): GHCR 패키지 `ros2-humble-slam`이 옛 repo(`ros2-humble-slam-docker`)에만 연결되어 Code_Space 워크플로우 push가 403 — 패키지 설정 Manage Actions access에 `PackagU/Code_Space`(Write) 추가 후 `gh run rerun 31289125731 --failed`.** 통과 시 humble-jetson digest를 이 메모에 기록할 것. 실기 잔여: udev VID/PID 2종(motor_nano/arm_servo) 벤치 확정, R1A/R1B 게이트, bag 재생 재SLAM 게이트(스펙 §5.5~5.6). 컨테이너 재생성됨(scripts 마운트 추가) — 다음 시뮬 실행 시 colcon 전체 빌드 1회 필요.
