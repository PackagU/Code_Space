...
권장 현장 조작은 [Field web console](06_field_web_console.md)을 사용한다. 브라우저 한 화면에서 실시간 지도, WASD, 매핑 시작·저장·종료, 저장 지도 Nav2를 제어할 수 있다. 아래 터미널 절차는 화면 서버를 쓰지 못할 때의 대체 경로다.
...
OpenCR와 Jetson 브리지의 바퀴 상한은 사용자 지정 `[제안값] 30 rpm`이다. 수동 텔레옵의 직진 0.10 m/s와 제자리 회전 0.35 rad/s는 각각 이 상한 아래다. Nav2는 곡선 주행의 합성 wheel RPM까지 고려해 `[제안값] 0.05 m/s`, `[제안값] 0.20 rad/s`로 더 낮췄다. 미탐색 영역 경로는 금지하며 실제 검증 전에는 올리지 않는다.
