...
ok=0
for i in $(seq 1 30); do grep -q "drive ready=True" $L && { ok=1; break; }; sleep 2; done
sleep 4
./scripts/fieldctl status < /dev/null 2>&1 | sed -n 1,8p
R=$(X "timeout 6 ros2 topic echo --once /drive/ready" | grep -c "data: true")
if [ "$ok" != 1 ] || [ "$R" != 1 ]; then echo "ABORT: drive not ready (log=$ok topic=$R)"; grep -E "opencr_bridge" $L | tail -6 | cut -c1-170; exit 1; fi
M=$(wc -l < $L)
./scripts/fieldctl resume < /dev/null 2>&1 | head -1
X "timeout 3.5 ros2 topic pub -r 20 /cmd_vel geometry_msgs/msg/Twist '{linear: {x: 0.10}, angular: {z: 0.25}}' > /dev/null & sleep 2.2; ros2 topic echo --once /odom --field twist.twist; wait"
