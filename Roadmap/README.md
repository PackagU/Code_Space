...
| 04 동적 장애물 | `[90%]` | 2026-08-21: **동적 보행자(참값 근접 판정 + yield) + 경로 위 정적 장애물 2개**(F1 복도 중앙·F2 뒤 복도 중앙, static) 하 왕복 체인 **10/10 연속 무결점**(재시도 0). 보행자↔로봇 교착은 yield(4s 후 후퇴/퇴장)로 해소. 남은 것: collision_monitor 도입 검토 |
| 05 엘베 내부 + 멀티층 | `[75%]` | 2026-08-21: 리프트(Z축) mock 동작(픽업/하차, /joint_states 검증) + F2 출구 스테이징 goal 추가. 엘베 칸 내부 모델링/도킹 정밀도 미완 |
| 06 통합 반복 안정성 | `[90%]` | 2026-08-21: 적대 리뷰 29건 반영 후 **최종 스택 데스크톱 10/10 연속 PASS(전 회차 재시도 0·REALIGN 0, 278~348s)** + 선행 4/4 (§1.26 바퀴 물리 근본 원인 = 무마찰 캐스터 크리프·fdir1 퇴화, §1.27 핫픽스 묶음). 남은 것: 고장 주입 5종 완결, 캐스터 마찰 실기 무관 확인 |
| 07 Jetson 배포 | `[80%]` | 2026-08-21: **분산 원커맨드(`scripts/run_distributed_e2e.sh`) 왕복 3/3 완주** (전 옵션, 각 11 goal 1차 성공·재시도 0, Jetson cpu avg 48%/peak 89~94%, **mem 2.5GB/6.8GB**, missed 23~29 → 분산 게이트 60). 남은 것: 실기 센서(RPLiDAR) 주행, missed-rate 실기 임계 수립 |
